...
export function buildNarrativeAnalysis(args: {
  ...
  if (pattern.category !== "变格") {
    ...
    // 格局专属感情基调（兜底：当感情文案不足2条时，补充格局本身的感情倾向说明）
    // 依据：《三命通会》各格局人际特征与《子平真诠》配偶星取用
    // 只有日支一条（六合/六冲等）时不够全面，补充格局感情基调使报告更完整
    if (relationships.length < 2) {
      ...
      const hint = patternRelHints[pattern.name];
      if (hint) relationships.push(hint);
    }
  }
