...
export function buildNarrativeAnalysis(args: {
  ...
  if (pattern.category === "变格") {
    ...
    if (pattern.outcome === "败格") {
      ...
      if (isOfficerMixed) {
        ...
        // 喜财制枭，运行财星则发"
        career.push("枭神压制食神，表达与创作天赋受阻；职场宜踏实做事，避免急于展示才艺，等财星大运到来制枭解食，才华方能得以施展。");
      ...
      // 败格通用补充第二条：用神候运方向（依《子平真诠》"候大运流年以期救格，
      // 喜用之运到来则格局可期翻转"）
      // 只在 career 尚不足2条时补充，避免与前面已有的大运提示重复
      if (career.length < 2) {
        const yongShenLabel = yongShen.yongShen.length > 0 ? yongShen.yongShen.join("、") : "喜用";
        career.push(`用神方向以${yongShenLabel}为主，大运流年走入喜用方向时，是格局获得补救、事业发力的窗口期。`);
      }
    } else {
      // ——成格/待观察：以十神偏重推断优势方向——
  ...
  if (pattern.category === "变格") {
    ...
    // 格局专属财富补充（当 wealthScore/peerScore 均偏低时，从格局角度给出财富走向）
    // 依据：《子平真诠》各格局对财的态度不同——印格忌财但财运来临仍需顺势
    if (wealthScore < 2 && peerScore < 2) {
  ...
  // 用神触发提示：仅在 wealth 中未出现"财星恰与用神方向重合"时输出，
  // 避免重复表达"用神=财星"的信息（财星重合文案已更精确）。
  // 依据：《子平真诠》"用神既明，财富触发点自然明了，无需再行叠说"
  const wealthAlreadyHasYongShenInfo = wealth.some(s => s.includes("财星恰与用神方向重合"));
  if (yongShen.yongShen.length > 0 && !wealthAlreadyHasYongShenInfo) {
    wealth.push(
  ...
  if (pattern.category !== "变格") {
    ...
    if (pattern.name === "偏印格" && !strength.isStrong) {
      ...
      relationships.push("命局气旺，感情中有能力主动表达和经营，但内心对情感安全感和被理解的需求同样强烈；需在主动付出与接受照顾间找到平衡。");
    } else {
      if (strength.isStrong) {
