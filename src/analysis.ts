...
export function buildNarrativeAnalysis(args: {
  ...
  // 特殊格局败格场景：overview 专属警示（依《子平真诠》"败格论，先明病根"）
  // 这些场景在命盘全局有深远影响，不只是事业维度的问题，需在 overview 层面先行揭示
  if (pattern.outcome === "败格" && pattern.category !== "变格") {
    const overviewBrokenReason = pattern.reasons.slice(-1)[0] ?? "";
    if (overviewBrokenReason.includes("伤官见官") || overviewBrokenReason.includes("伤官损官")) {
      // 依据：《子平真诠》"伤官见官，天下第一凶险，官不能用，伤官横扫一切秩序"
      overview.push("⚠️ 命局出现「伤官见官」——伤官之气与官星直接对冲，在格局层面属于凶险组合；此局不宜依赖官星（体制、规则、权威）作为人生主轴，宜以自由、创业、技艺等路径扬长避短，方能化格局凶险为个人优势。");
    } else if (overviewBrokenReason.includes("七杀旺而无") || overviewBrokenReason.includes("遇杀无制")) {
      // 依据：《子平真诠》"七杀无制，杀星肆虐，性急冒进，谋事须蓄势"
      overview.push("⚠️ 命局出现「七杀无制」——七杀之力旺盛而无食神/印星制化，格局层面偏于冒进激烈；行事需蓄势待时，避免锋芒毕露，待大运走入制杀或化杀之运后，格局方能稳定发力。");
    } else if (overviewBrokenReason.includes("官杀混杂")) {
      // 依据：《子平真诠》"官杀混杂，两星相争，方向不定，格局力量分散"
      overview.push("⚠️ 命局出现「官杀混杂」——正官与七杀同透，格局方向分散，人生容易在「规则内晋升」与「竞争突破」之间摇摆；宜选定一个主线深耕，待大运合化使格局理清，方能集中力量取得成就。");
    } else if (overviewBrokenReason.includes("枭神夺食")) {
      // 依据：《三命通会》"枭神夺食，才艺难全，表达受阻，命局失去最自然的泄秀通道"
      overview.push("⚠️ 命局出现「枭神夺食」——偏印（枭神）压制食神，命局中最自然的才艺表达通道受阻；才华需要在压力和约束中磨砺，当下宜踏实积累、不急于表现，待财星制枭的大运到来后，才华才能真正释放变现。");
    } else if (overviewBrokenReason.includes("比肩透出且无官星") || overviewBrokenReason.includes("劫财透出且无官星")) {
      // 依据：《子平真诠》"财格见比劫而无官，财旺亦难久守，命局以分财论"
      overview.push("⚠️ 命局出现「比劫夺财」——财格中比劫旺而无官星制衡，财源虽宽但难以独享；命局提示需警惕合伙分利、资源流失，宜独立经营、守护核心财路，候官星制劫之运方能稳固财源。");
    } else if (overviewBrokenReason.includes("未见官杀制刃")) {
      // 依据：《子平真诠》"羊刃无制，刚烈难驯，意外风险偏高，需等官杀运驾驭"
      overview.push("⚠️ 命局出现「羊刃无制」——羊刃之力旺盛而无官杀驾驭，刚烈冲动之气难以收束；命局整体偏于急进冲动，需特别注意意外风险与情绪管理，候官杀运到来方能将刃力转化为竞争优势。");
    } else if (overviewBrokenReason.includes("正官透出且无印星化解")) {
      // 依据：《子平真诠》"伤官格见官，宜印化解；无印则伤官与官直冲，格局大凶"
      overview.push("⚠️ 命局出现「伤官见官（无印化解）」——伤官格中正官透出而无印星化解，属格局中的重大破格；此局尤忌官场与体制，宜远离规则约束型环境，以创业、自由职业或专业技艺为主线，候印星运到来可解格局危机。");
    }
  }
    ...
    if (pattern.outcome === "败格") {
      ...
      if (career.length >= 2) {
        ...
        const careerKeyFactors: Record<string, string> = {
          ...
          // 《子平真诠》"正印格，官印相生，行官运时印力得用，名誉与地位最容易提升"
          "正印格": "大运走入官星或印星旺运时，资历积累、资格认证与社会认可度大幅提升，是晋升与扩大影响力的关键窗口，宜提前布局积累。",
  ...
  if (pattern.category === "变格") {
    ...
    if (wealthScore >= 2) {
      if (isResourcePatternBroken) {
        ...
        // 候运建议已在 career 维度覆盖，此处聚焦当下财务策略，避免重复
        // 依据：《子平真诠》"印格遭财破，宜守本分、以技能立身，不宜冒险投资或轻易经营"
        wealth.push("印格遭财破，财星反成干扰而非助力；理财策略宜保守稳健，避免涉足高风险投资或多线经营，以技能专业积累为主，财务上量入为出即可。");
      } else if (isWealthPatternBroken) {
        // 财格败格（比劫夺财）：财星活跃但已被比劫分夺，应警示而非正向推荐
        // career维度已覆盖「候官星制劫大运」建议，此处聚焦当下防守型财务策略，不重复候运
        // 依据：《子平真诠》"正财格，比劫为大忌，财旺无官制，宜少合伙、独守财路"
        wealth.push("比劫夺财之局，财星虽活跃却难独享；财务上宜独立管账、明确产权边界，减少不必要的合伙或资金共管，避免辛苦所得被分摊。");
  ...
  if (pattern.category !== "变格") {
    // 特殊格局败格场景：relationships 专属感情分析
    // 依据：《子平真诠》各格败格对感情关系的具体影响；《三命通会》"官伤情感，杀混婚乱"
    // 这些场景的感情影响有格局特异性，需在通用旺弱底色之前先行揭示
    if (pattern.outcome === "败格") {
      const relBrokenReason = pattern.reasons.slice(-1)[0] ?? "";
      if (relBrokenReason.includes("伤官见官") || relBrokenReason.includes("伤官损官")) {
        // 依据：《子平真诠》"伤官见官，命局凶险，感情上同样逆官星而动"
        // 依据：《三命通会》"伤官旺者，感情直接，易因言语刺伤关系；女命伤官见官，克夫之象"
        relationships.push("「伤官见官」格局在感情层面同样有显著影响——伤官旺者情感表达直接锐利，在亲密关系中容易因言辞或个性触怒对方；女命此格更需留意「克官」之象，感情中宜收敛锋芒、多包容少争辩，方能维护关系稳定。");
        if (gender === "female") {
          // 依据：《三命通会》"女命伤官见官，以克夫论，婚姻多有波折，宜印运化解"
          relationships.push("女命伤官见官，传统命理以「克夫」论，婚姻感情更需以包容和耐心经营；大运走入印星或官星沉静之时，关系才真正趋向稳定和谐。");
        }
      } else if (relBrokenReason.includes("官杀混杂")) {
        // 依据：《子平真诠》"官杀混杂，方向不定，感情上易两情交织、优柔寡断"
        // 依据：《三命通会》"官杀并现，心中双线，感情难以专注"
        relationships.push("「官杀混杂」格局中，官星（规则、责任、长期）与七杀（激情、冲动、短期）并存，感情模式容易出现理性与冲动交织、难以做出清晰承诺的特征；宜在确定的关系框架中明确边界，避免在多段感情之间摇摆消耗。");
        if (gender === "male") {
          // 依据：《三命通会》"男命官杀混杂，财星被两星争夺，感情中双重压力并存"
          relationships.push("男命官杀混杂，财星（配偶星）受官杀两方拉扯，感情上容易同时承受多方压力或责任；宜专注经营核心关系，减少分散投入。");
        }
      } else if (relBrokenReason.includes("七杀旺而无") || relBrokenReason.includes("遇杀无制")) {
        // 依据：《子平真诠》"七杀无制，冲动行事，感情亦然，来去激烈"
        // 依据：《三命通会》"七杀旺者，感情中主导欲强，冲撞关系"
        relationships.push("「七杀无制」的命局中，七杀之力冲撞无阻，感情关系容易带有强烈的冲击性——情感表达激烈、进退快速；宜有意识地放慢节奏，在亲密关系中培养耐心与包容，减少冲动决策对感情的冲击。");
      } else if (relBrokenReason.includes("枭神夺食")) {
        // 依据：《三命通会》"枭神夺食，才艺受压，感情中表达能力受抑，难以自然流露情感"
        // 依据：《子平真诠》"食神受压，生活情趣与才华输出均受阻，感情中缺少润滑"
        relationships.push("「枭神夺食」格局中，食神（生活情趣、才艺表达、情感滋养）被偏印压制，感情表达的自然流通受阻；亲密关系中容易出现情感表达不畅或生活情趣枯燥的问题，宜主动创造轻松愉快的共处空间，以行动而非言辞传递情感。");
      } else if (relBrokenReason.includes("比肩透出且无官星") || relBrokenReason.includes("劫财透出且无官星")) {
        // 依据：《子平真诠》"财格见比劫，财被分夺，配偶星（财星）同受比劫侵犯"
        // 依据：《三命通会》"劫财旺者，感情经济基础不稳，婚姻关系易因财务纠纷动摇"
        relationships.push("「比劫夺财」的命局中，财星（男命配偶星）受比劫分夺，婚姻关系容易因经济基础不稳或财务分配问题而产生摩擦；宜在感情关系中提前明确财务边界，建立稳固的经济基础，减少因金钱问题引发的关系危机。");
      } else if (relBrokenReason.includes("未见官杀制刃")) {
        // 依据：《子平真诠》"羊刃无制，刚烈冲动，感情中易因急躁伤人"
        // 依据：《三命通会》"羊刃主孤，无制时克伤配偶之象较重"
        relationships.push("「羊刃无制」格局中刚烈之气无法收束，感情中容易因冲动言行、情绪爆发而伤害亲密关系；宜有意识地锻炼情绪管理能力，在亲密关系中以「退一步」的方式降低冲突烈度，候官杀运以驾驭刃力后感情才真正趋稳。");
      } else if (relBrokenReason.includes("财星破印") || relBrokenReason.includes("财星透出，印星受克")) {
        // 依据：《子平真诠》"印格遭财破，精神层面受物质干扰，感情重精神共鸣的需求难以满足"
        relationships.push("「印格遭财破」的命局中，印星（精神涵养、情感滋养）受财星干扰，感情上对精神共鸣的深度需求与现实物质压力常产生冲突；宜在关系中维护精神与情感空间，减少让物质利益主导情感决策。");
      }
    }
  ...
  // 特殊格局败格场景：health 专属健康补充（依《三命通会》"格局影响五脏气机"）
  // 这些场景对健康有格局特异性影响，在通用五行脏腑建议后追加专属提示
  if (pattern.outcome === "败格" && pattern.category !== "变格") {
    const healthBrokenReason = pattern.reasons.slice(-1)[0] ?? "";
    if (healthBrokenReason.includes("伤官见官") || healthBrokenReason.includes("伤官损官")) {
      // 依据：《三命通会》"伤官格见官，两星相冲，气机郁结，肝气最易受损"
      // 依据：《黄帝内经》"郁怒伤肝，肝主疏泄，抑郁则气滞"
      health.push("「伤官见官」格局中，伤官之气被官星压制，气机容易郁结于内；长期处于压抑状态对肝胆（主情志疏泄）影响尤大，容易表现为情绪积压、烦闷易怒或肝气不舒；宜多从事舒展性运动、艺术创作等疏泄情志的方式来保护身心。");
    } else if (healthBrokenReason.includes("七杀旺而无") || healthBrokenReason.includes("遇杀无制")) {
      // 依据：《三命通会》"七杀无制，杀气肆虐日主，神经与心脏首当其冲"
      // 依据：《黄帝内经》"恐伤肾，惊伤心，七杀主克伐，长期紧绷伤神"
      health.push("「七杀无制」格局中，杀气持续克伐日主，身体长期处于紧张对抗状态；易引发心脏（主神志）和肾脏（主意志）的耗损，表现为睡眠质量差、精神紧绷或抗压储备下降；宜以静养为主，避免长期高压透支神气。");
    } else if (healthBrokenReason.includes("官杀混杂")) {
      // 依据：《子平真诠》"官杀混杂，两力相争，身体承受双重压力"
      // 依据：《黄帝内经》"多虑伤脾，分心散气，脾胃失调"
      health.push("「官杀混杂」格局中，两种克力同时作用，精神层面容易出现多虑、纠结和方向感缺失；长期内耗易影响脾胃（主思虑运化）和神经系统，表现为消化不稳定或决策后疲惫感；宜减少过度思虑，专注当下降低精神内耗。");
    } else if (healthBrokenReason.includes("枭神夺食")) {
      // 依据：《三命通会》"枭神夺食，食神受压，脾胃（食神主消化）首当其冲"
      // 依据：《子平真诠》"食神为喜神，受枭神克，身体对应区域失调"
      health.push("「枭神夺食」格局中，食神被偏印（枭神）压制，食神对应的脾胃、消化系统容易受到影响；常见表现为食欲不稳定、消化吸收欠佳或口腔问题；宜规律饮食、不暴饮暴食，重视脾胃养护，待财星制枭运到来后整体健康状态才能明显改善。");
    } else if (healthBrokenReason.includes("比肩透出且无官星") || healthBrokenReason.includes("劫财透出且无官星")) {
      // 依据：《子平真诠》"财格见比劫，财星受分，精力在争夺与分配中大量消耗"
      // 依据：《三命通会》"比劫夺财，心神不宁，常有患得患失之感"
      health.push("「比劫夺财」格局中，财星持续受比劫分夺，心理层面容易出现患得患失、对资源安全感不足的焦虑；长期处于担忧得失的状态易消耗心脾之气，表现为思虑过重、脾胃欠佳或睡眠质量不稳；宜减少对财务得失的过度关注，以规律生活维护身心稳定。");
    } else if (healthBrokenReason.includes("未见官杀制刃")) {
      // 依据：《子平真诠》"羊刃无制，刚烈冲动，意外受伤风险偏高"
      // 依据：《三命通会》"羊刃主金刃之伤，无制则伤险倍增；主行车意外、手术刀刃之类"
      health.push("「羊刃无制」格局中，刃力旺盛而无制约，意外受伤（刀伤、行车事故、运动创伤等）的风险明显偏高；宜在日常生活中保持谨慎，避免过度冒险性活动；金刃相关手术建议选择大运走官杀制刃之时，风险相对可控。");
    } else if (healthBrokenReason.includes("财星破印") || healthBrokenReason.includes("财星透出，印星受克")) {
      // 依据：《子平真诠》"印格遭财破，印星（主肾脑记忆）受损，精神与脑力首先受影响"
      // 依据：《黄帝内经》"肾藏志，脑为髓海，印格受损则精力与记忆下降"
      health.push("「印格遭财破」的命局中，印星（对应肾脑与精神涵养）受财星干扰，容易出现精力分散、脑力下降或睡眠质量不稳的问题；宜注重脑力恢复与肾气养护，减少过度劳神的工作安排，待印旺运到来后精神状态才能明显提振。");
    } else if (healthBrokenReason.includes("正官透出且无印星化解")) {
      // 依据：《子平真诠》"伤官见官无印，两星相冲无化解，情志最易积郁成疾"
      // 依据：《黄帝内经》"怒伤肝，思伤脾，伤官见官情志失调，肝脾为主要影响"
      health.push("「伤官见官（无印化解）」格局中，气机郁结最为严重，情志长期压抑容易积郁成疾；肝（情志疏泄）与脾（思虑运化）是最需要关注的健康重点，宜积极寻找情志出口（创作、运动、倾诉），避免长期情绪压制对身体的慢性损耗。");
    }
  }
