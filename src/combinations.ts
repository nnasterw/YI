...
export function detectCombinations(profile: BaziProfile, isStrong: boolean): TenGodCombination[] {
  ...
  if (biJie >= 3 && caiXing >= 1) {
    combos.push({
      type: "比劫夺财",
      detected: true,
      strength: Math.min(10, biJie * 2),
      personality: "有人在争你的资源——不一定是恶意的，但你挣的钱总有人来分。独立意识极强，不喜欢被安排。",
      career: "合伙慎重！利益分配一定要写清楚。独立运营比合伙经营更适合你。如果必须合伙，你要控股。",
      relationship: "在感情中有'护食'倾向。比劫也代表同性竞争——你的伴侣可能有桃花被他人觊觎。",
      risk: "逢劫财流年破财风险高。不要借钱给朋友、不做担保、不匆忙签合同。"
    });
  }
