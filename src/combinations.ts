...
export function detectCombinations(profile: BaziProfile, isStrong: boolean): TenGodCombination[] {
  ...
  // 9. 财官相生
  // 依据：《子平真诠》"官星最要财来助"——财生官、官护财，正格中最受推崇的良性结构，
  // 且两者需同时天干透出才为"显格"；仅藏干有之则力弱，不作标记。
  // 身强时此结构最能发挥（有余力承担财官之责）；身弱时财官皆为耗泄之神，结构虽好
  // 但日主承受力不足，会呈现外表风光内里吃力的特征。
  if (hasStemTenGod(profile, ["正财", "偏财"]) && hasStemTenGod(profile, ["正官", "七杀"])) {
    combos.push({
      type: "财官相生",
      detected: true,
      strength: isStrong ? 8 : 5,
      personality: isStrong
        ? "你有能力把资源转化为权力、把实力换来地位。不是那种在体制外漂泊的人，而是天生能在有秩序的系统里往上走。"
        : "财官结构看起来好，但身弱时会感觉撑得很辛苦——外界对你期望不低，你却总觉得资源和精力都差一口气。",
      career: isStrong
        ? "最适合大公司、体制或有品牌的组织。在清晰的晋升通道里做实事，财官相生能让你一路向上。不适合一开始就'自己当老板'。"
        : "更适合找到一个稳定平台先扎根，再谈扩展。财官都旺而身弱时，强行扛一摊容易积劳成疾。",
      relationship: isStrong
        ? "在亲密关系中你是靠谱的那个：经济稳定、有担当、不乱来。但也容易把关系经营得过于'务实'，忘了浪漫。"
        : "你对伴侣有责任感，但压力大时也容易把情绪带回家。需要一个能接住你疲态的伴侣。",
      risk: isStrong
        ? "财官皆旺时，容易陷入'只顾向上走、忽视人情温度'的困境。注意别把职业成功感带成人际冷漠。"
        : "身弱财官旺——最怕大运双重走财官，届时压力会集中爆发，需要提前蓄力或找印星化解。"
    });
  }

  // 10. 印比双旺
  // 依据：《子平真诠》"比重无财，愚蒙之辈"——身强本已不缺帮扶，印星再源源不断生身，
  // 加上比劫成群，形成"过旺无泄"的壅塞格局，能量只进不出，容易固执、孤僻、难以变通。
  // 身强时此结构为负面提醒；身弱时印比本是喜神，不在此判断范围内。
  if (isStrong && yinXing >= 2 && biJie >= 2) {
    combos.push({
      type: "印比双旺",
      detected: true,
      strength: Math.min(10, (yinXing + biJie) * 2),
      personality: "你的内部能量很足，但出口堵着——想法多、自我意识强、不太需要外人认可。这本来不是坏事，但过旺时容易变成固执：'我觉得对的就是对的，别人说什么都没用'。",
      career: "你不适合单纯服从的工作，也不适合反复被质疑的环境——印比双旺的人需要一定的自主权。但最大的坑是：把自信变成听不进意见。需要食伤和财星来逼你向外输出和验证。",
      relationship: "你在关系中有强烈的'自我'——不一定强势，但很难完全放下主见。伴侣如果太顺着你，你可能反而少了成长；真正需要一个能'怼'你的人。",
      risk: "《子平真诠》：比重无财，流于空谈。印比双旺若无食伤财官来疏泄，天赋和积累很容易只停留在内部、不落地为成果。最需要的不是再学什么，而是逼自己去做、去输出。"
    });
  }

  // 11. 财克印（财多印弱）
  // 依据：《子平真诠》"财多印弱，早年不利"——财星克制印星，意味着内在的支持系统
  // （学习力、贵人缘、家庭保护、直觉判断）被财欲所损，常表现为轻文重商、贵人缘淡薄、
  // 考运或文书事务不顺。两者都天干透出时信号最强，仅藏干有之则力弱。
  // 身强时财为克泄喜神而印为忌神，财克印反而是好事（财克掉忌神），不在此讨论范围内；
  // 身弱时印是喜神，被财所克是真正的不利结构。
  if (!isStrong && hasStemTenGod(profile, ["正财", "偏财"]) && hasStemTenGod(profile, ["正印", "偏印"])) {
    combos.push({
      type: "财多印弱",
      detected: true,
      strength: 5,
      personality: "你有挣钱的欲望，但内在的保护和支持系统相对薄弱——遇事容易孤立无援，或在追逐外部利益时忽略了对自身积累的维护。",
      career: "适合在实操中成长，不适合走纯学历/文凭/考证路线——财克印意味着传统的'读书晋升'通路对你阻力更大，直接闯反而更顺。但不建议过早放弃知识积累，否则后劲不足。",
      relationship: "贵人缘和扶持系统相对薄弱，遇到困难时更多要靠自己解决。学会主动寻求帮助、不总是'硬撑'，是重要的成长功课。",
      risk: "财克印最直接的风险是忽视积累、只顾短线。身弱还财旺，资源虽多但消耗大，容易两手一直是空的。大运如果再走财星，需提前留意健康和贵人的流失。"
    });
  }
